    ...
    <CustomEventComponent title="交易对账" :tableData="tableData" :total="total">
      <template #search>
        <div class="search">
          <el-form :inline="true">
            ...
            <el-form-item >
              <el-select v-model="formInline.type" clearable style="width:200px" placeholder="校对结果">
          ...
          <div v-if="route.params.type!='all'">
            <el-button type="primary" class="add" @click="addData()" round>重新发起所有误差请求</el-button>
          </div>
        </div>
      </template>
      <template #table="{ tableData }">
        ...
        <el-table :data="tableData">
          <el-table-column label="交易日期" prop="id" />
          <el-table-column label="校对时间" prop="name" />
          <el-table-column label="请求积分交易笔数" width="180">
          ...
          <el-table-column label="请求权益交易笔数" width="180" prop="updatedTime" />
          <el-table-column label="实际权益交易笔数" width="180" prop="createTime" />
          <el-table-column label="实际处理积分交易笔数" width="180" prop="createBy" />
          <el-table-column label="实际权益交易笔数" width="180" prop="usedCount">
            <template #default="scope">
              <el-tag class="mx-1" :type="statusLabels[scope.row.status].type?statusLabels[scope.row.status].type:'info'" effect="light">
                {{ statusLabels[scope.row.status].Text }}
              </el-tag>
          ...
          <el-table-column label="操作" width="280" fixed="right">
            <template #default="scope">
              <el-space wrap>
                ...
                <el-link type="primary" @click="detailsData(scope.row)">查看误差详情</el-link>
              </el-space>
            </template>
          </el-table-column>
        </el-table>
